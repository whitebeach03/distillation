    ...
    for i in range(1):
        ...
        for epoch in range(epochs):
            ...
            if score <= val_acc:
                print('save param')
                score = val_acc
                torch.save(model.state_dict(), './logs/resnet/' + str(i) + '.pth') ######## 
        ...
        with open('./history/resnet/' + str(i) + '.pickle', mode='wb') as f: #########
            pickle.dump(history, f)

        # model test
        model.load_state_dict(torch.load('./logs/resnet/' + str(i) + '.pth'))
        model.eval()
